...
from application.posts.forms import CreateRecipeForm, FindRecipeForm
...
@posts.route("/create_recipe", methods=['GET', 'POST'])
def create_recipe():
...
@posts.route("/recipe_catalog", methods=['GET', 'POST'])
def recipe_catalog():
    if not session.get("cookie"):
        return flask.redirect('login')
    else:
